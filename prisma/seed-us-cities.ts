...
async function fetchAndSeed() {
  console.log("Fetching real data for US walking cities...");
  // We won't delete existing data so we keep what we have, or maybe we do delete if we want a fresh start.
  // Let's delete to ensure clean data for this massive pull
  ...
  for (const city of walkingCities) {
    ...
    try {
      ...
      for (const place of data.places) {
        ...
        // Use the city name from our list if parsing is messy
        const finalCity = city || city.split(",")[0];
        ...
        await prisma.bathroom.create({
          data: {
            ...
            isVerified: true, // Data from Google is assumed verified initially
            verificationCount: 1
          }
        });
        totalInserted++;
      }
      console.log(`✅ Seeded ${data.places.length} locations for: ${city}`);
    } catch (e) {
      console.error(`Error querying ${query}`, e);
    }
  }
  
  console.log(`🎉 Finished! Successfully inserted ${totalInserted} bathrooms across the US.`);
}
